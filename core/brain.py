...
def _construir_prompt_skills(skills_disponibles: dict) -> str:
...
def _extraer_json(texto: str) -> Optional[dict]:
    """Intenta extraer un JSON válido del texto, incluso si viene con texto extra."""
    texto = texto.strip()
    # Intento directo
    try:
        ...
        pass
    # Buscar bloque JSON con regex (entre llaves exteriores)
...
def procesar_orden(texto_usuario: str, skills_disponibles: dict) -> dict:
    """
    Envía el texto del usuario al LLM (Ollama local) y parsea la respuesta
    ...
    skills_descripcion = _construir_prompt_skills(skills_disponibles)
    ...
    for intento in range(1, 3):  # hasta 2 intentos
        ...
        if parsed is not None:
            # Validar que la skill exista en el registro
            skill_pedida = parsed.get("skill")
            if skill_pedida and skill_pedida not in skills_disponibles:
